...
import { componentOptions, statOptions } from '@/types/OptionsValue';
import DatePicker from 'react-datepicker';
import 'react-datepicker/dist/react-datepicker.css';
 

export default function TambahDataBackLogForm() {
  ...
  // Manage dynamic part rows
  const { fields, append, remove } = useFieldArray({
    control,
    name: 'parts'
  }); 
   
  const [activeTab, setActiveTab] = useState(0); // keep track of which tab is active


  const roleString = String(role); 
  const statusOption =
    roleString === "Non Staff" ? [statOptions[0]] : statOptions;
  ...
  const onSubmit = async (dataBackLog: BackLog) => {  
    ...
    dataBackLog.parts.forEach((part, index) => {
      if (!part.part_number.trim()) {
        setError(`parts.${index}.part_number`, { type: "manual", message: "Part Number wajib diisi" });
        hasError = true;
      }
      if (!part.part_description.trim()) {
        setError(`parts.${index}.part_description`, { type: "manual", message: "Part Description wajib diisi" });
        hasError = true;
      }
      if (isNaN(part.qty_order) || part.qty_order <= 0) {
        setError(`parts.${index}.qty_order`, { type: "manual", message: "Qty Order wajib diisi" });
        hasError = true;
      }
    });
        ...
        <div className="mt-8 w-full">
          ...
          <div className="flex justify-between items-center border-b mb-4 w-full">
            ...
            <button
              type="button"
              onClick={() => {
                append({ part_number: "", part_description: "", qty_order: 1 });
                setActiveTab(fields.length); // move focus to the newly added tab
              }}
              className="ml-4 px-4 py-2 text-sm bg-green-500 text-white rounded hover:bg-green-600 transition"
            >
              + Add Part
            </button>
          </div> 
          ...
          {/* Tab Content - Show only active part */}
          {fields.map((field, index) => (
            <div
              key={field.id}
              className={`transition-opacity duration-300 ease-in-out ${activeTab === index ? "block opacity-100" : "hidden opacity-0"}`}
            >
              <div className="p-2 bg-gray-50 shadow-sm">
                {/* Part Number */} 
                <div className="mb-3">
                  <InputFieldsLabel
                    label="Part Number"
                  ...
                  />
                  {errors.parts?.[index]?.part_number && (
                    <p className="text-red-500 text-sm">
                      {errors.parts[index].part_number?.message}
                    </p>
                  )} 
                </div>
                ...
                <div className="mb-3">
                  <InputFieldsLabel
                    ...
                    type="text"
                    {...register(`parts.${index}.part_description` as const, {
                        required: "Part Description wajib diisi",
                      })} 
                  />
                  {errors.parts?.[index]?.part_description && (
                    <p className="text-red-500 text-sm">
                      {errors.parts[index].part_description?.message}
                    </p>
                  )}  
                </div>

                {/* Quantity */}  <div className="mb-3">
                  <InputFieldsLabel
                    label="Qty"
                    type="text"
                    {...register(`parts.${index}.qty_order` as const, {
                      required: "Qty Order wajib diisi",
                      valueAsNumber: true,
                      min: { value: 1, message: "Minimal 1" },
                    })}
                  />
                  {errors.parts?.[index]?.qty_order && (
                    <p className="text-red-500 text-sm">
                      {errors.parts[index].qty_order?.message}
                    </p>
                  )}  
                </div> 
