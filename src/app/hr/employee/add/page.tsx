...
export default function TambahDataFuelRatioForm() { 
  ...
  const onSubmit = async (formData: EmployeeAddDetailEditFormValue) => {  
    ...
    if (isNaN(formData.position_id) || formData.position_id <= 0) {
      setError("position_id", {type: "manual", message: "Posisi wajib diisi"});
      hasError = true;
    }
    ...
    if (!formData.level || formData.level.trim() === '') {  
      setError("level", {type: "manual", message: "Kategori Statistik K3 wajib diisi"});
      hasError = true;
    } 

    if (isNaN(formData.role_id) || formData.role_id <= 0) {
      setError("role_id", {type: "manual", message: "Level wajib diisi"});
      hasError = true;
    }  
    ...
    formData.DOH = formData.DOH.map((doh) => ({
      ...doh,
      tanggal_doh: normalizeDate(doh.tanggal_doh),
    })); 
    
    formData.Sertifikat = formData.Sertifikat.map((s) => ({
      ...s,
      date_effective: normalizeDate(s.date_effective),
    }));

    formData.MCU = formData.MCU.map((mcu) => ({
      ...mcu,
      date_mcu: normalizeDate(mcu.date_mcu),
    }));

    formData.History = formData.History.map((h) => ({
      ...h,
      tanggal: normalizeDate(h.tanggal),
    }));
    ...
    <div className="relative mx-auto"> 
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center w-full mb-4 gap-4"> 
          <ContentHeader className="m-0" title="Tambah Data Employee" /> 
