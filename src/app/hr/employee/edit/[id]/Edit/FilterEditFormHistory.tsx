...
interface FilterEditFormHistoryProps {
  onApply: (filters: {
...
const FilterEditFormHistory: React.FC<FilterEditFormHistoryProps> = ({ onApply, onReset, defaultValues }) => {
  const {
    control,
    handleSubmit,
    register,
  ...
  useEffect(() => {
    if (defaultValues) {
      reset({
        status_terakhir: defaultValues.status_terakhir || '',
        tanggal:
          defaultValues.tanggal && typeof defaultValues.tanggal === 'string'
            ? new Date(defaultValues.tanggal)
            : defaultValues.tanggal ?? null,
...
export default FilterEditFormHistory;
