...
import { penempatanOptions, kontrakOptions, ptOptions } from '@/types/OptionsValue';
 
interface FilterEditFormKontrakProps {
  onApply: (filters: {
    pt: string;
    tanggal_doh: string;
...
const FilterEditFormKontrak: React.FC<FilterEditFormKontrakProps> = ({ onApply, onReset, defaultValues }) => {
  const {
    control,
    handleSubmit,
    register,
  ...
  if (defaultValues) {
    reset({
      pt: defaultValues.pt || '',
      doh_date:
        defaultValues.doh_date && typeof defaultValues.doh_date === 'string'
          ? new Date(defaultValues.doh_date)
          : defaultValues.doh_date ?? null,
      tanggal_end_doh:
        defaultValues.tanggal_end_doh && typeof defaultValues.tanggal_end_doh === 'string'
          ? new Date(defaultValues.tanggal_end_doh)
          : defaultValues.tanggal_end_doh ?? null,
      penempatan: defaultValues.penempatan || '',
      status_kontrak: defaultValues.status_kontrak || '',
    });
  }
}, [defaultValues, reset]);
  ...
  return (
    <>
      <h2 className="!text-start text-xl font-bold mb-6">Ubah Data DOH</h2>
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="col-span-3 grid grid-cols-3 items-start gap-4">
            ...
            <div className="col-span-2 space-y-1 !text-start">
              <Controller
                control={control}
                name="doh_date"
...
export default FilterEditFormKontrak;
