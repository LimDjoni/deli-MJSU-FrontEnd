...
interface FilterEditFormSertifikatProps {
  onApply: (filters: {
    sertifikat: string;
...
const FilterEditFormSertifikat: React.FC<FilterEditFormSertifikatProps> = ({ onApply, onReset, defaultValues }) => {
  const {
    control,
    handleSubmit,
    register,
  ...
  useEffect(() => {
    if (defaultValues) {
      reset({
        sertifikat: defaultValues.sertifikat || '',
        date_effective:
          defaultValues.date_effective && typeof defaultValues.date_effective === 'string'
            ? new Date(defaultValues.date_effective)
            : defaultValues.date_effective ?? null, 
  ...
  return (
    <>
      <h2 className="text-xl font-bold mb-6">Ubah Data Sertifikat</h2>
...
export default FilterEditFormSertifikat;
