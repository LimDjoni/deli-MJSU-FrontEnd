...
export default function DetailDataBarangKeluartForm({ params }: { params: Promise<{ id: string }> }) {
...
const updateFlag = getUpdateFlag(menuItems, '/transaction/barang-keluar');
const deleteFlag = getDeleteFlag(menuItems, '/transaction/barang-keluar');
  ...
  const handleDelete = async (dataAsset: BarangKeluar) => {
    try {
      await MJSUAPI({
      ...
      const updatePayload = { 
        ...
        stock: dataAsset.Asset.stock + dataAsset.jumlah_keluar,
      };
  ...
  return (
    ...
    <div className="relative mx-auto"> 
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center w-full mb-4 gap-4"> 
          <ContentHeader className="m-0" title="Detail Data Barang Keluar" /> 
          <div className="flex gap-2">
            <ButtonDisabled type="button" 
            onClick={() => router.push('/transaction/barang-keluar/')}
            className="px-6">
              Kembali
            </ButtonDisabled> 
            ...
            <ButtonAction type="submit" 
              ...
              onClick={() => updateFlag && router.push(`/transaction/barang-keluar/edit/${id}`)}
            >
              Ubah
            </ButtonAction>
          </div>
