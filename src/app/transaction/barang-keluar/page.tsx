...
import { BarangKeluar } from '@/types/AssetValues';
...
const createFlag = getCreateFlag(menuItems, '/transaction/barang-keluar');
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await MJSUAPI({
          url: `/barangkeluar/list/pagination?${query}`,
          method: 'GET',
          headers: {
  ...
  return (
    <div>
      <ContentHeader className="mx-auto" title="Data Barang Keluar" />
      <div className="flex justify-between items-center w-full mb-4">
        ...
        <ButtonAction
          ...
          onClick={() => createFlag && router.push('/transaction/barang-keluar/add/')}
          icon={<Plus size={24} />}
        >
          Buat Baru
        </ButtonAction>
        ...
        <table className="min-w-full text-left border-collapse">
          <thead>
            <tr className="bg-[#FF3131] text-white text-center">
              ...
              <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('employee_id')}>
                Karyawan {sortField === 'employee_id' && (sortDirection === 'asc' ? '↑' : '↓')}
              </th>
              <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('department_id')}>
                Department {sortField === 'department_id' && (sortDirection === 'asc' ? '↑' : '↓')}
              </th>
              ...
              <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('jumlah_keluar')}>
                Jumlah Keluar {sortField === 'jumlah_keluar' && (sortDirection === 'asc' ? '↑' : '↓')}
              </th> 
            </tr>
          </thead>
          <tbody>
            {BarangKeluarList.length > 0 ? (
              BarangKeluarList.map((barang, index) => (
                <tr key={barang.ID} className="border-t text-center">
                  <td className="px-4 py-2 text-[#FF3131] underline cursor-pointer hover:font-bold"
                      onClick={() => router.push(`/transaction/barang-keluar/detail/${barang.ID}`)}>
                    {(page - 1) * limit + index + 1}
                  </td>
                  <td className="px-4 py-2">{barang.employee.lastname == "" ? barang.employee.firstname : `${barang.employee.firstname} ${barang.employee.lastname}`}</td>
                  <td className="px-4 py-2">{barang.employee.Department.department_name}</td>
                  ...
                  <td className="px-4 py-2">{barang.jumlah_keluar}</td> 
                </tr>
              ))
            ) : (
              <tr>
