...
import { Asset, BarangMasuk } from '@/types/AssetValues';
import DatePicker from 'react-datepicker';
import ErrorMessage from '@/components/ErrorMessage';
import 'react-datepicker/dist/react-datepicker.css';

export default function EditDataBarangMasuktForm({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const [prevJumlahMasuk, setPrevJumlahMasuk] = useState<number>(0);

  const { register, handleSubmit, setValue, control, formState: { errors } } = useForm<BarangMasuk>({
    mode: 'onSubmit',
    defaultValues: {
      tanggal: null,
      asset_type_id: '', 
    ...
    const fetchDetail = async () => {
      try {
        ...
        if (data) {
          ...
          setPrevJumlahMasuk(jumlah_masuk);
          ...
          setValue('jumlah_masuk', jumlah_masuk); 
        }
      } catch (error) {
  ...
  const onSubmit = async (dataAsset: BarangMasuk) => {
    try {
      // Step 1: Update Barang Masuk record
      const updatePayload = {
        asset_id: dataAsset.Asset.ID,
        tanggal: normalizeDate(dataAsset.tanggal),
        jumlah_masuk: dataAsset.jumlah_masuk,
      };

      const updateResponse = await MJSUAPI({
        url: `/barangmasuk/update/${id}`,
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
          "content-type": "application/json",
      ...
      console.log("BarangMasuk Updated:", updateResponse.data);

      // Step 2: Update Asset stock
      // Formula: initial stock - prev jumlah_masuk + new jumlah_masuk
      const finalStock = dataAsset.Asset.stock - prevJumlahMasuk + dataAsset.jumlah_masuk;
  ...
  return ( 
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="relative mx-auto">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center w-full mb-4 gap-4">
        <ContentHeader className="m-0" title="Ubah Data Barang Masuk" />
        <div className="flex gap-2">
          <ButtonDisabled
            type="button"
            onClick={() => router.push(`/transaction/barang-masuk/detail/${id}`)}
            className="px-6"
          >
            Kembali
          </ButtonDisabled>
